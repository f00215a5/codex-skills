...
本技能的 QA 有兩道關卡：**標註語意 QA**（在組 DOCX 之前）與 **DOCX 結構 QA**（`verify_docx.py`）。本版**不進行** Word／LibreOffice 視覺渲染驗證，所以紅框座標的正確性必須在圖片層級完成，不能靠任何後端轉檔來「補救」。
...
**紅框、編號、圖說三者一致是獨立的交付條件**。沒有 Word 等後端可以再次修正 raw 圖上的座標錯誤：若 raw 座標就錯，嵌入 DOCX 只會把同一個錯誤等比帶進文件。
