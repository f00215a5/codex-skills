...
- CSV 沒有的既有問題單依確認規則轉為「不明」；保留顯示的前導零、人工備註、格式、公式與篩選。既有 hidden 狀態只保留在未填入問題單的資料列；所有 populated issue rows 在交付前都必須有效可見。
...
- 輸出已儲存且 workbook writer 完全關閉後，會從磁碟唯讀重開 artifact，獨立驗證資料正確性、可見性、公式快取與真實渲染。四層均通過才稱完整成功；沒有可用 renderer 時會明確回報 `PARTIAL`。
